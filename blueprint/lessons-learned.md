...
L11 — Отрицание перепроверяй СМЕНОЙ КЛЮЧЕВОГО СЛОВА: ищи механизм, а не имя практики.
  Why: в «Разрыве» мы написали «сравнительных измерений эффекта fitness-функций не нашли» — и это
  было неверно. Измерения есть (Кнодель/Мутиг/Рост, шесть команд, ICSM 2008; Ольссон и соавторы,
  восемь проектов, ECSA-W 2016), просто обе работы старше термина и слов «fitness function» не
  употребляют — нашлись по «architecture conformance checking». Правило: прежде чем писать «мы не
  нашли», прогони запрос ещё раз по механизму (что физически делает практика), по её старому имени
  и по смежной дисциплине. Формулировка «мы не нашли» от ошибки не спасает — она её публикует.
  Anchor: pieces/20260924_ai-delivery-gap/fact-check.md (F9, раунд 3) / sources/INDEX.md#architecture-conformance

L12 — Служебные ярлыки плана помечай непубликуемыми — иначе они уезжают в прозу дословно.
  Why: зеркало поймало в черновике «Мост:», «тут обязателен стейлмен», «прививка от проповеди» —
  мои же рабочие пометки из `outline.md`, перенесённые писателем в текст как есть. Писатель не
  обязан отличать инструкцию себе от фразы для читателя: если ярлык стоит в строке структуры, он
  часть текста. Правило: в плане либо явная пометка «[не в текст]» у каждого служебного ярлыка,
  либо формулировка задачи фразой («здесь одно предложение для того, кто впервые слышит термин»),
  которую нельзя принять за готовую прозу.
  Anchor: pieces/20260924_ai-delivery-gap/mirror.md / tasks/20260923_sdlc-ai-pitfalls/log.md

L13 — Мягкий перенос строки в markdown → `<br>` на WordPress: конвертер обязан отдавать его пробелом.
  Why: исходники свёрстаны по ~95 символов; `wpautop()` в WordPress превращает каждый такой перенос
  в `<br>`, и опубликованный текст ломается ровно по местам вёрстки — посреди фразы, посреди
  ссылки. Починено в `scripts/wp_publish.py`: правило рендера `softbreak` отдаёт пробел (на блоки
  кода не влияет — у них свои токены). Общее: перед публикацией сверяй не только HTML из
  `--dry-run`, но и то, что с ним сделает сам движок.
  Anchor: scripts/wp_publish.py / wiki/topics/publishing-to-tellian.md
