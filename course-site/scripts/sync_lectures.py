...
PUBLISHED = ["lec-01", "lec-02", "lec-03", "lec-04", "lec-05"]
LANGS = ["ru", "en"]
DPI = 120
...
def strip_stage_directions(body: str) -> str:
    ...
    # редакторские сноски-переходы между частями речи (не для читателя): «*Продолжение —
    # [speech-part2.md](…)*» (lec-03) или «*Продолжение — `speech-part2.md`: …*» (lec-05).
    # Убираем всю строку с упоминанием файла speech*.md (линк / код-спан / текст) — это
    # всегда внутренняя ссылка, не контент для читателя (иначе битый линк + служебка в паблике).
    body = re.sub(r'(?mi)^.*\bspeech[\w.-]*\.md\b.*$\n?', '', body)
