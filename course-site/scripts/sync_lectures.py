...
def strip_stage_directions(body: str) -> str:
    ...
    # редакторские сноски-переходы между частями речи (не для читателя):
    # «*Продолжение — [speech-part2.md](speech-part2.md): …*», ссылки на другие speech-файлы.
    # Убираем строку целиком (иначе битый внутренний линк + служебный текст в паблике).
    body = re.sub(r'(?mi)^.*\]\((?:\./)?speech[\w.-]*\.md[^)]*\).*$\n?', '', body)
