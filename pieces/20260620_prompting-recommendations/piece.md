...
Пять приёмов дальше — пять способов распорядиться этими двумя бюджетами. Но один и тот же приём в живом чате и во «вшитом» в продукт промпте стоит по-разному: где-то промах — копейки, а где-то множится на миллионы вызовов. Поэтому держим в уме четыре типа задач — у каждого своя цена ошибки и свой запас по бюджетам:
...
Дальше у каждого приёма помечаю, где он решает, а где им можно пренебречь.

---

## Приём 1 · 🌐 Язык и токенизация

Выбор языка — рычаг цены и качества, а не объёма: сжать промпт сменой языка не выйдет, зато легко переплатить и потерять в точности. На западных моделях английское ядро инструкций обычно и дешевле, и точнее — но обращаться к модели можно на любом удобном языке, и это нормально работает.
...
Внимание модели достаётся краям окна, а середина проваливается — поэтому важное и сам вопрос ставьте по полюсам, не в середину. И не рассчитывайте на всё окно: надёжной длины меньше, чем в спеке. Чем длиннее промпт, тем дороже ошибка с местом.
...
Ради этого всё и затевалось — собираем рычаги под конкретную задачу.
...
### Сквозные решения — что выбрать в спорном случае

Эти вопросы встают в любом типе задач, и ответ на них не зависит от того, чат у вас или агент:
