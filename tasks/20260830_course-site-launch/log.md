...
## 2026-08-30 — RU-доступ решён (Moscow VPS) + контент обновлён + ребрендинг/дизайн
- **RU-доступ:** зарубежные edge все режутся/троттлятся из РФ (Vultr-AMS блок; Vultr-FRA reachable но
  задушен «только заголовок таба»; Gcore/CF edge блок). Решение — **российский origin**: VPS
  88.218.62.36 (Hosting technology LTD, Moscow). Перенёс весь стек (site+Umami+Remark42+Caddy). Из РФ
  открывается быстро. DNS lessons.tellian.io A → 88.218.62.36. **HTTPS (Let's Encrypt) на домене.**
  Gcore/CF/Frankfurt — отброшены (не подошли).
- **Контент обновлён из lessons main (7e97552):** lec-01..04 **двуязычно (RU+EN)**, чистый speech (cue
  убраны), **lec-04 теперь строится** (41 секция). Генератор: language-aware (speech.en.md/lec-NN-en.pdf,
  naming-маппинг), published-only 01-04 (05-17 = draft, пауза), нумерация слайдов в подписях.
- **Ребрендинг:** «AI-usage-lessons» → **«Использование ИИ в различных индустриях»** / EN «AI Across
  Industries» (site_name per-lang).
- **Дизайн:** нумерованный правый TOC + CSS левого меню/TOC (отступы, выделение активного, рубрики).
- **GA** (G-99D2ZE94Y7) + **Umami** (website-id 6e69fbba) + **Remark42** (анонимный логин по имени).
- Telegram-логин комментариев **не вышел**: с этого VPS api.telegram.org недоступен (v4/v6, timeout) —
  убрал; модерацию сделаем другим логином (владелец: не Яндекс, не Телеграм).
- **Хвосты:** 3 упоминания «семинар» вплетены в реплики lec-01 (правка исходника sibling); footer-less
  деки от sibling ещё не в main; admin-логин для модерации; 05-17 держим в draft.

## 2026-08-30 — КОРЕНЬ «некорректных картинок»: рассинхрон PDF + content-mapping
- Владелец: слайды/картинки некорректны (напр. «70 лет AI» показывал дивайдер «Раздел 1»).
- Диагноз (сверка текст-слоя PDF vs deck/speech): merge-скрипт lessons ВЕРНУЛ удалённые бауманские
  слайды (Карта семестра/Оценка за семестр/Чат-MAX-QR) на стр.7-9 lec-01 → сдвиг + локальные swap'ы.
  Только lec-01 затронута; lec-02/03/04 чисты (подтвердил sibling).
- Фикс (B, robust): маппинг «комментарий → слайд» переписан с позиционного на **content-based
  выравнивание (DP, как diff)** по тексту слайда: пропускает вставленные лишние страницы, держит
  обложки/Q&A на диагонали (чистые лекции не ломаются), + rescue-проход для локальных swap'ов.
  Бауманские секции дропаются целиком (exclude по конфигу) + их страницы исключены из rescue.
- Также: strip_stage_directions (убраны ремарки «[На слайде—…]/[Backup…]»); публикуются только 01-04.
- Итог: lec-01 33/34 (Карта/Оценка/Чат-QR убраны, «70 лет AI»→timeline с 1956); lec-02/03/04 идеально
  (35/35, 40/40, 40/40). 1 секция lec-01 («Чат: модель+интерфейс») без слайда — чинит pub-PDF sibling.

## 2026-08-30 — pub-PDF подхвачены: lec-01 34/34, footer-less, финал
- Sibling запушил footer-less pub-PDF (PR #181, f12eee5). Генератор (lecture_files) уже предпочитает
  lec-NN-pub.pdf → подхватил автоматически. lec-01-pub (34 стр., без бауманских, s17 «Чат» восстановлен).
- **lec-01 RU+EN: 34/34** (content-mapping чисто, без-пары=[]). Футеры «N/NN» убраны (pub footer-less).
  «Карта семестра/Оценка» нет, «70 лет AI»=timeline с 1956, комментарии на своих слайдах. lec-02/03/04 —
  тоже из pub (footer-less), целы. Проверено live.
- Хвост (не критично): lec-01-pub из канонического build без on-slide [N]-цитат (были только в merge-PDF);
  комментарии-источники в speech на месте. Follow-up при желании владельца.

## 2026-09-06 — Лекция 2 v3.0 (47 слайдов) опубликована + генератор укреплён
- Контент-сессия (ae39b1f9) пересобрала lec-02: новая структура, 47 слайдов/100 мин, RU+EN parity,
  speech.md/speech.en.md v3.0 с [sNN]-якорями, deck.yaml/deck.en.yaml с assertion. Владелец дал команду
  публиковать обе версии.
- Обнаружены 3 «мины» при обновлении лекции — закрыты обобщённо в sync_lectures.py (commit a6ff3e8):
  1) stale lec-02-pub.pdf (35с, v1.x из моего же PR #181) подменял бы свежий дек → choose_ru_pdf()
     берёт pub только если он покрывает все секции speech (индекс истины), иначе full + срез футеров;
  2) RU-full дек с футерами «N/NN» → strip_footer_pagenums() редактирует нижнюю полосу (no-op на чистых);
  3) диалект «### [sNN · dur]» без названия → подпись из deck.yaml assertion по id (load_deck_titles).
- Deploy: rsync site/ → RU VPS 88.218.62.36:/var/www/course-site (ключ переустановлен bootstrap'ом).
- Проверено live из РФ: lec-02 RU+EN=47/47 footer-less, подписи человеческие, images 200; регрессий нет
  (lec-01=34, lec-03=40, lec-04=40; лендинг=4 карточки).
- ⚠ На флаг владельцу: слайды lec-02 содержат интернет-мемы (s01 — кадр Aardman «Pirates!»). Публичный
  бренд-сайт → риск по правам. Владелец в курсе, команда дана; вынесено на решение (снимается флагом).
- MCP cao-server отключён — ответить сиблингу ae39b1f9 через send_message нельзя; координация через владельца.

## 2026-09-07 — Лекция 3 v6.3 (55 слайдов) опубликована RU+EN
- Контент-сессия переработала lec-03 → v6.3: 55 слайдов, многочастная структура
  (speech.md+speech-part2.md, deck.yaml+part2+part3), не-цифровые id (s-classic-*, s22a_multi), мемы.
- Генератор укреплён под это (commits 94115a4, 29fa30a, PR #23):
  1) многочастная речь/дек склеиваются (_ordered_parts/speech_files/deck_files/parse_speech_all);
  2) _SID=r's[\d-][\w-]*' — распознаём s-classic-*/s22a_multi, подпись из deck assertion;
  3) INSTRUCTOR_NOTE_RE — обрезаем «Заготовки ответов»/prep/резерв внутри слайда;
  4) вырезаем меж-частные ссылки на speech*.md (ломали mkdocs --strict + утечка служебки).
- Блокер по пути: RU rendered/lec-03.pdf был 51с (не хватало 5 слайдов «Классическая база» —
  headline-фича v6.3). Завёл баг контент-сессии AI-usage-lessons#193 → они пересобрали (PR #194,
  51→55с). Владелец решил держать обе и публиковать вместе.
